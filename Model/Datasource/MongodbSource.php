...
// This comes from composer
use Composer\InstalledVersions;

// This comes from the composer module mongodb/mongodb.
...
class MongodbSource extends DboSource
{
	...
	{
		$this->_driverVersion = InstalledVersions::getPrettyVersion('mongodb/mongodb') ?? 'unknown';
		...
		try
		{
			...
			if (isset($this->config['replicaset']) && count($this->config['replicaset']) === 2)
			{
				$this->connection = new $class(
					$this->config['replicaset']['host'],
					$this->config['replicaset']['options']
				);
			}
			...
			else if ($this->_driverVersion >= '1.19.0')
			{
				$this->connection = new $class($host, array(), $this->config);
			}
			else
			{
				$this->connection = new $class($host, true, $this->config);
			}
			...
			if (isset($this->config['slaveok']))
			{
				...
				{
					$this->connection->setReadPreference(
						$this->config['slaveok'] ? $class::RP_SECONDARY_PREFERRED : $class::RP_PRIMARY
					);
				}
			}
